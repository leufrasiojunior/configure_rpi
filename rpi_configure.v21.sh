...
update_package_cache() {
  ...
  if [ ! "${today}" == "${timestampAsDate}" ]; then
    ...
    if eval "${UPDATE_PKG_CACHE}" &> /dev/null & spinner $!; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
    else
...
systemctl daemon-reload &> /dev/null & spinner $!
systemctl enable $daemonname.service &> /dev/null & spinner $!
shortcutfile
}
...
install_dependent_packages "tasksel" &> /dev/null & spinner $!
tasksel install samba-server &> /dev/null & spinner $!
...
chown -R "${install_user}":"${install_user}" /var/www/html
    ...
    # Create a command from the package cache variable
    if systemctl restart smbd &> /dev/null & spinner $!; then
        printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}\n"
    else
        # Otherwise, show an error and exit
        printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
...
######## FIRST CHECK ########
    ...
    DebConfphp
        ...
        if service apache2 restart &> /dev/null & spinner $!; then
            printf "%b  %b %s\\n" "${OVER}" "${TICK}" "${str}"
        else
            printf "%b  %b %s\\n" "${OVER}" "${CROSS}" "${str}"
